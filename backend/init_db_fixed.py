...
def create_schema(conn):
    ...
    cur.executescript("""
        ...
        CREATE TABLE IF NOT EXISTS mall_flow_data (
            ...
            visitor_people INTEGER NOT NULL DEFAULT 0,
        ...
        CREATE INDEX IF NOT EXISTS idx_mall_flow_mall_date ON mall_flow_data(mall_id, data_date);
        CREATE INDEX IF NOT EXISTS idx_mall_flow_date ON mall_flow_data(data_date);
...
def import_data(conn):
    ...
    for _, row in df_mall.iterrows():
        ...
        if not data_date:
            continue
        try:
            cur.execute("""INSERT OR IGNORE INTO mall_flow_data 
                (mall_id, data_date, visitor_count, visitor_people, avg_stay_minutes, 
                 male_ratio, female_ratio, age_18_24_ratio, age_25_34_ratio, age_35_44_ratio, age_45_plus_ratio)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (mall_id, data_date,
                 int(row['商场顾客人次']), int(row['商场顾客人数']),
                 float(row['商场平均停留时长(分钟)']),
                 float(row['男性顾客占比(%)']), float(row['女性顾客占比(%)']),
                 float(row['18-24岁顾客占比(%)']), float(row['25-34岁顾客占比(%)']),
                 float(row['35-44岁顾客占比(%)']), float(row['45岁以上顾客占比(%)'])))
            mall_count += 1
        except Exception as e:
            print(f"  Skip mall flow: {mall_id} {data_date} - {e}")
    conn.commit()
    print(f"Imported {mall_count} mall flow records")
...
def compute_indices(conn, cur):
    ...
    # 商场指数
    mall_idx_count = 0
    cur.execute("SELECT DISTINCT mall_id FROM mall_flow_data")
    malls = [r[0] for r in cur.fetchall()]
    cur.execute("SELECT DISTINCT data_date FROM mall_flow_data ORDER BY data_date")
    ...
    for mall_id in malls:
        for d in mall_dates:
            ...
            cur.execute("SELECT visitor_count FROM mall_flow_data WHERE mall_id=? AND data_date=?", (mall_id, d))
            row = cur.fetchone()
            if not row or row[0] is None:
                continue
            actual = row[0]
            
            cur.execute("SELECT visitor_count FROM mall_flow_data WHERE mall_id=? AND data_date=?", (mall_id, prev_yr_date))
            baseline_row = cur.fetchone()
            if not baseline_row or baseline_row[0] is None or baseline_row[0] == 0:
                continue
            baseline = baseline_row[0]
...
if __name__ == "__main__":
    ...
    tables = ['brands', 'cities', 'business_districts', 'malls', 'stores', 
              'mall_flow_data', 'store_flow_data', 'store_flow_index', 'mall_flow_index',
